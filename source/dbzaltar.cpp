...
void _add_special_anchor_postfix_CZ_hymnus_cezrok() {
	strcat(_anchor, _special_anchor_postfix_CZ_hymnus_cezrok);
}
...
// CZ OCARM (25MAJ)
void _set_zalmy_vesp_113_122_ef(short int modlitba) {
	Log("_set_zalmy_vesp_113_122_ef(%s) -- begin\n", nazov_modlitby(modlitba));
	if (modlitba == MODL_VESPERY) {
		set_zalm(1, modlitba, "z113.htm", "ZALM113");
		set_zalm(2, modlitba, "z122.htm", "ZALM122");
		set_zalm(3, modlitba, "ch_ef1.htm", "CHVAL_EF1");
	}
	Log("_set_zalmy_vesp_113_122_ef(%s) -- end\n", nazov_modlitby(modlitba));
}// _set_zalmy_vesp_113_122_ef()
...
void _vlastne_slavenie_hymnus(short int modlitba, char vlastny_anchor[SMALL], short int litobd, short int pouzit_special_anchor /* = ANO */) {
	...
	if (pouzit_special_anchor == ANO) {
		sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, vlastny_anchor, pismenko_modlitby(modlitba), ANCHOR_HYMNUS);
	}
	else {
...
void _velk1_hymnus(short int den, short int modlitba, short int litobd) {
	...
	if (ktory < 0) {
		sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, _anchor_tmp, pismenko_modlitby(modlitba), ANCHOR_HYMNUS);
	}
	else {
		// upravené kotvy, aby bolo použiteľné zjednodušene toto:
		sprintf(_anchor, "%s%s_%c%s%d", _special_anchor_prefix_CZ_hymnus, _anchor_tmp, pismenko_modlitby(modlitba), ANCHOR_HYMNUS, ktory);
	}
...
#define _adv_hymnus {\
	sprintf(_anchor, "%s%s%c_%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[litobd], pismenko_modlitby(modlitba), ANCHOR_HYMNUS);\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
	}\
...
// switch(litobd), case OBD_ADVENTNE_I -- begin -----------------------------------------------
	case OBD_ADVENTNE_I :// do 16. decembra
			...
			if (den == DEN_NEDELA) { // nedeľa: 1.-3. adventná nedeľa (okrem prípadu, kedy 3. adventná nedeľa padne na 17. decembra, ako napr. v r. 2006 alebo 2023)
				...
				modlitba = MODL_PRVE_VESPERY;
				sprintf(_anchor, "%s%s%c_%s", _special_anchor_prefix_CZ_hymnus,
					nazov_OBD[litobd],
...
#define _bohorod_hymnus(vlastny_anchor) {\
	...
	sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, vlastny_anchor, c, ANCHOR_HYMNUS);\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
	}\
...
#define _vian1_hymnus {\
	...
	if ((modlitba == MODL_PRVE_VESPERY) && (_global_den.den != 25))\
		c = pismenko_modlitby(MODL_VESPERY);\
	sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[OBD_VIANOCNE_I], c, ANCHOR_HYMNUS);\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
	}\
...
#define _vian2_hymnus {\
	sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[OBD_VIANOCNE_II], pismenko_modlitby(modlitba), ANCHOR_HYMNUS);\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
		}\
...
#define _zjv_hymnus {\
	sprintf(_anchor, "%s%s_%s%c%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[OBD_VIANOCNE_II], ANCHOR_ZJAVENIE_PANA, pismenko_modlitby(modlitba), ANCHOR_HYMNUS);\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
		}\
...
#define _post1_hymnus {\
	if ((den == DEN_NEDELA)\
		...
		sprintf(_anchor, "%s%s_%c%s%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[litobd], pismenko_modlitby(modlitba), nazov_DN_asci[den], ANCHOR_HYMNUS);\
	}\
	else {\
		sprintf(_anchor, "%s%s_%c%s", _special_anchor_prefix_CZ_hymnus, nazov_OBD[litobd], pismenko_modlitby(modlitba), ANCHOR_HYMNUS);\
	}\
	if (modlitba == MODL_POSV_CITANIE) {\
		_set_hymnus(modlitba, _file_pc, _anchor);\
		set_LOG_litobd_pc;\
...
// switch(litobd), case OBD_VELKONOCNA_OKTAVA -- end ------------------------------------------
	...
	if ((PODMIENKA_MOZE_BYT_SPOMIENKA_PM_V_SOBOTU) && (poradie_svateho == PORADIE_PM_SOBOTA)) {
		...
		modlitba = MODL_RANNE_CHVALY;
		// hymnus
		sprintf(_anchor, "%s%s%c_%s", _special_anchor_prefix_CZ_hymnus, SPOM_PM_SOBOTA, pismenko_modlitby(modlitba), ANCHOR_HYMNUS);
		_set_hymnus(modlitba, _file, _anchor);
		set_LOG_litobd;
		// krátke čítanie
...
void _spolocna_cast_hymnus_rozne(short int modlitba, char *_anchor_pom, char *_anchor, char *_file, int force) {
	if (su_inv_hymnus_kcit_kresp_benmagn_prosby_vlastne(modlitba) || ((force & FORCE_BRAT_HYMNUS) == FORCE_BRAT_HYMNUS)) {
		sprintf(_anchor, "%s%s%c%s", _special_anchor_prefix_CZ_hymnus, _anchor_pom, pismenko_modlitby(modlitba), ANCHOR_HYMNUS);
		_set_hymnus(modlitba, _file, _anchor);
		set_LOG_svsv;
	}
}
...
_struct_lang_anchor_and_count pocet_hymnus_multi_anchor_count[] = {
	...
	{ JAZYK_SK, "19MAR_1HYMNUS", 2 },
	{ JAZYK_SK, "01MAJ_vHYMNUS", 2 },
	{ JAZYK_SK, "16MAJ_vHYMNUS", 2 },
...
_struct_lang_cal_type_anchor_and_count pocet_multi_lang_cal_type_anchor_count[] = {
	...
	{ JAZYK_CZ, KALENDAR_CZ_OPRAEM, BASE_OPT_6_HYMNUS_MULTI, "CZ_06JUN_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OPRAEM, BASE_OPT_6_HYMNUS_MULTI, "02MAJ2_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OPRAEM, BASE_OPT_6_HYMNUS_MULTI, "CZ_02MAJ2_vHYMNUS", 2 },
	...
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "09AUG_cHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "09AUG_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "09AUG_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_MODLITBA_MULTI, "09AUG_MODLITBA", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "17AUG_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "25AUG5_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01SEP_cHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01SEP_rHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01SEP_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "01OKT_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01OKT_cHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01OKT_rHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "01OKT_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "15OKT_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "15OKT_cHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "15OKT_rHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "15OKT_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_MODLITBA_MULTI, "05NOV_MODLITBA", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "06NOV_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_MODLITBA_MULTI, "06NOV_MODLITBA", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "08NOV_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "14NOV_cCIT2", 3 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_KCIT_RESP_MULTI, "14NOV_rCIT", 3 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_KCIT_RESP_MULTI, "14NOV_rRESP", 3 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14NOV_vHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_ANTIFONA_MULTI, "14NOV_vMAGNIFIKAT", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14DEC_cHYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_CITANIE2_MULTI, "14DEC_cCIT2", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14DEC_9HYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14DEC_2HYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14DEC_3HYMNUS", 2 },
	{ JAZYK_CZ, KALENDAR_CZ_OCARM, BASE_OPT_6_HYMNUS_MULTI, "14DEC_vHYMNUS", 2 },
