					...
					if(equals(strbuff, PARAM_REFERENCIA_END) && (vnutri_inkludovaneho == 1)){
						...
						if((_global_opt[OPT_0_SPECIALNE] & BIT_OPT_0_REFERENCIE) == BIT_OPT_0_REFERENCIE){
							...
							if((refrest != NULL) && !(equals(refrest, STR_EMPTY))){
								...
								if(EXPORT_REFERENCIA){
									Export("%s", refrest); // 2013-06-12: pÙvodne sa odstraÚovala diakritika; remove_diacritics(refrest)
								}
							}// naËÌtanie na zaËiatok referencie
							if(EXPORT_REFERENCIA){
								Export("%s\" target=\"_blank\" "HTML_CLASS_QUIET">", refbuff); // a.quiet { text-decoration:none; color: inherit; } // 2013-06-12: pÙvodne sa odstraÚovala diakritika; remove_diacritics(refbuff)
							}
						}
						if(EXPORT_REFERENCIA){
							Export("%s", refbuff);
