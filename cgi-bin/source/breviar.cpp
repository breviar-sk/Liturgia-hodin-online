...
void _export_link_show_hide(short int opt, int bit_opt, char popis_show[SMALL], char popis_hide[SMALL], char html_tag_begin[SMALL], char html_class[SMALL], char specific_string_before[SMALL], char specific_string_after[SMALL], char anchor[SMALL], char html_tag_end[SMALL]){
	...
	int _global_opt_backup = _global_opt[opt];
			...
			// 2011-10-07: zakončenie v texte includovanej modlitby
				...
				if (equals(strbuff, PARAM_KATECHEZA_END) && (vnutri_inkludovaneho == 1)){
					...
					if (isGlobalOption(OPT_0_SPECIALNE, BIT_OPT_0_REFERENCIE)){
						...
						if (EXPORT_REFERENCIA){
							Export("\" "HTML_TARGET_BLANK" "HTML_CLASS_QUIET">"); // a.quiet { text-decoration:none; color: inherit; }
						}
					}
					if (EXPORT_REFERENCIA){
...
void interpretParameter(short int type, char *paramname, short int aj_navigacia = ANO){
	...
	// voliteľné NEzobrazovanie celého hymnu (ostane len nadpis v podobe rubrikového small caps headingu) nižšie nasleduje samotné vypísanie textu chválospevu Te Deum, ak _global_opt[OPT_1_CASTI_MODLITBY]...
		...
		if ((isGlobalOption(OPT_1_CASTI_MODLITBY, BIT_OPT_1_TEDEUM) || isGlobalOption(OPT_0_SPECIALNE, BIT_OPT_0_BLIND_FRIENDLY)) && (_global_opt_tedeum == ANO) && (_global_skip_in_prayer != ANO)){
#if defined(EXPORT_HTML_SPECIALS)
	...
	if((typ != EXPORT_DNA_VIAC_DNI) && (typ != EXPORT_DNA_VIAC_DNI_SIMPLE) && (typ != EXPORT_DNA_VIAC_DNI_TXT) && (typ != EXPORT_DNA_XML)){
		...
		if(den_zoznam == ANO){
			...
			if(som_v_tabulke == ANO){
				...
				Export("<"HTML_TABLE_CELL">\n");
				Export(HTML_COMMENT_BEGIN"_global_string_farba(1)"HTML_COMMENT_END);
			}
		...
		if((som_v_tabulke == ANO) && (isGlobalOption(OPT_2_HTML_EXPORT, BIT_OPT_2_BUTTONY_USPORNE))){
			...
			// JUV: these two added experimentally!
			Export(HTML_COMMENT_BEGIN"table-end-1"HTML_COMMENT_END);
			Export(HTML_TABLE_END"\n");
			Export("<"HTML_TABLE">\n");
		...
		if (isGlobalOption(OPT_2_HTML_EXPORT, BIT_OPT_2_BUTTON_PRVE_VESPERY)){
			...
			if (
				...
				// oddelenie riadka
				if ((som_v_tabulke == ANO) && (isGlobalOption(OPT_2_HTML_EXPORT, BIT_OPT_2_BUTTONY_USPORNE))){
					Export(HTML_COMMENT_BEGIN"table-row"HTML_COMMENT_END);
					Export("<"HTML_TABLE_ROW">\n");
					Export("<"HTML_TABLE_CELL">\n");
				}
...
#define ZOBRAZ_BUTTON_DETAILY
		...
		if(typ == EXPORT_DNA_VIAC_DNI){
			...
			if(som_v_tabulke == ANO){
				...
				Export("<"HTML_TABLE_CELL">\n");
				Export(HTML_COMMENT_BEGIN"_global_string_farba(2)"HTML_COMMENT_END);
				Export("%s", _global_string_farba);
				Export(HTML_TABLE_CELL_END"\n");
...
void _export_rozbor_dna_buttons_dni_compact(short int typ, short int dnes_dnes /* = ANO */){
	...
	if ((_global_pocet_navigacia <= 1) && (_global_pocet_volani_interpretTemplate < 2)){
		...
		if ((query_type == PRM_DATUM) && (_global_modlitba != MODL_NEURCENA)){
			...
			_export_rozbor_dna_buttons_dni_dnes(ANO /* dnes_dnes */, ANO /* som_v_tabulke */, pom2, ANO /* zobraz_odkaz_na_skrytie */);
	...
	if ((typ != EXPORT_DNA_VIAC_DNI) && (typ != EXPORT_DNA_VIAC_DNI_SIMPLE) && (typ != EXPORT_DNA_VIAC_DNI_TXT)){
		...
		if (dnes_dnes == ANO){
			...
			if (_global_opt_batch_monthly == NIE){
				...
				Export(HTML_A_HREF_BEGIN"\"%s\" "HTML_CLASS_QUIET_SMALL">(%s)"HTML_A_END""HTML_SPAN_END, pom, html_text_option_skryt[_global_jazyk]);
				Export("\n");
				Export(HTML_DIV_TABLE_CELL_END"\n");
...
void _export_rozbor_dna_kalendar_core(short int typ){
	...
	if ((typ != EXPORT_DNA_VIAC_DNI) && (typ != EXPORT_DNA_VIAC_DNI_SIMPLE) && (typ != EXPORT_DNA_VIAC_DNI_TXT)){
		...
		Export("<"HTML_TABLE_ROW">\n");
		Export(HTML_COMMENT_BEGIN"(header)"HTML_COMMENT_END"\n");
		...
		Export("<"HTML_TABLE_ROW">\n");
		Export(HTML_COMMENT_BEGIN"(line-1)"HTML_COMMENT_END"\n");
		...
		for (i = 1; i <= pocet_dni[_global_den.mesiac - 1]; i++){
			if (i == _global_den.den){
				if (((i + _global_den.denvt - _global_den.den) MOD 7) == 0){
					...
					Export("<"HTML_TABLE_ROW">\n");
					Export(HTML_COMMENT_BEGIN"(line-next)"HTML_COMMENT_END"\n");
					// nedeľa
...
void _export_main_formular(short int den, short int mesiac, short int rok, short int denvt){
	...
	Export(HTML_COMMENT_BEGIN"language selection combobox"HTML_COMMENT_END"\n");
	...
	Export(HTML_COMMENT_BEGIN"submit button (language)"HTML_COMMENT_END"\n");
	...
	Export(HTML_COMMENT_BEGIN"heading"HTML_COMMENT_END"\n");
	...
	if (zobrazit_moznosti1){ // len ak NIE JE možnosť (skrytie options1) zvolená
		...
		if((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_HU)){
			...
			Export(HTML_COMMENT_BEGIN"propria (proper calendars)"HTML_COMMENT_END"\n");
		...
		if(!isGlobalOptionForce(OPT_2_HTML_EXPORT, BIT_OPT_2_ROZNE_MOZNOSTI)){ // len ak NIE JE táto možnosť (zobrazovanie všeličoho) zvolená
			...
			Export(HTML_COMMENT_BEGIN"options-1"HTML_COMMENT_END"\n");
		...
		Export(HTML_COMMENT_BEGIN"options-2"HTML_COMMENT_END"\n");
		...
		Export(HTML_COMMENT_BEGIN"options-3"HTML_COMMENT_END"\n");
		...
		Export(HTML_COMMENT_BEGIN"options-4"HTML_COMMENT_END"\n");
		...
		Export(HTML_COMMENT_BEGIN"option-0"HTML_COMMENT_END"\n");
...
void _export_rozbor_dna_interpretuj_zoznam(short int export_typ, short int typ, short int som_v_tabulke, char batch_command[MAX_STR], short int modlitba, short int d_from_m_from_r_from){
	...
	for (int i = 1; i <= pocet; i++){
		...
		if (export_typ == EXPORT_TYP_BATCH_MODE){
			...
			if ((i > 1) && (typ != EXPORT_DNA_XML)){
				if (typ == EXPORT_DNA_VIAC_DNI_SIMPLE){
					...
					if (((typ == EXPORT_DNA_DNES) || (typ == EXPORT_DNA_JEDEN_DEN) || (typ == EXPORT_DNA_JEDEN_DEN_LOCAL)) && (isGlobalOption(OPT_2_HTML_EXPORT, BIT_OPT_2_BUTTONY_USPORNE))){
						// JUV: these two added experimentally!
						Export(HTML_COMMENT_BEGIN"table-end-2"HTML_COMMENT_END);
						Export(HTML_TABLE_END"\n");
						Export("<"HTML_TABLE">\n");
					}

					Export(HTML_COMMENT_BEGIN"row-iz"HTML_COMMENT_END);
			...
			Export(HTML_COMMENT_BEGIN"_export_rozbor_dna_interpretuj_zoznam"HTML_COMMENT_END);
...
void _export_rozbor_dna(short int typ){
	...
	Export(HTML_COMMENT_BEGIN"col:day_number"HTML_COMMENT_END"\n");
	if (som_v_tabulke == ANO){
	...
	Export(HTML_COMMENT_BEGIN"col:day_name"HTML_COMMENT_END"\n");
	...
	Export(HTML_COMMENT_BEGIN"col:buttons"HTML_COMMENT_END"\n");
	if (som_v_tabulke == ANO){
		Export("<"HTML_TABLE_CELL">\n");
	}
	Export(HTML_COMMENT_BEGIN"_export_rozbor_dna_zoznam follows"HTML_COMMENT_END);
	...
	if (typ == EXPORT_DNA_VIAC_DNI){
		...
		if (som_v_tabulke == ANO){
			Export(HTML_TABLE_CELL_END"\n");
			Export(HTML_COMMENT_BEGIN"col:roman_number"HTML_COMMENT_END"\n");
			Export("<"HTML_DIV_TABLE_CELL_VALIGN_TOP">\n");
		}
		else{
			Export(HTML_NONBREAKING_SPACE);
...
void _export_rozbor_dna_batch(short int typ, short int modlitba = MODL_NEURCENA, short int d_from_m_from_r_from = 0){
	...
	Export(HTML_COMMENT_BEGIN"_export_rozbor_dna_zoznam from batch follows"HTML_COMMENT_END);
	_export_rozbor_dna_zoznam(typ);
	// Log_zoznam();
...
#define _local_modl_kompletorium (*_local_modl_kompletorium_ptr)
	...
	if ((modlitba == MODL_VESPERY) || (modlitba == MODL_KOMPLETORIUM)){
		...
		_local_den = _global_den;
		_local_spol_cast = _global_opt[OPT_3_SPOLOCNA_CAST];
