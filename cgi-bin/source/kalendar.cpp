...
short int sviatky_svatych(short int den, short int mesiac, short int poradie_svaty){
	...
	// dalsia cast v zatvorkach {, } je sice len pre pripad, ze poradie_svaty == 1, 2, 3; 
	// avsak aj pre 0 resp. UNKNOWN_PORADIE_SVATEHO je to preto, ze aj ked nie je svaty urceny, moze ist o slavnost, ktora ma takmer najvacsiu prioritu, 
	// a preto ma aj prve vespery - a vtedy by to normalne nefungovalo
	{// pasaz kt. ma zmysel len pre poradie_svaty == 1, 2, 3, ale v pripade slavnosti aj pre == 0 resp. UNKNOWN_PORADIE_SVATEHO
	...
	case MES_JAN:
		...
		case 21: // MES_JAN -- 21JAN
			if (poradie_svaty == 1){
				...
				modlitba = MODL_RANNE_CHVALY;
				_set_zalmy_1nedele_rch();
				if (_global_jazyk == JAZYK_CZ){ // 2012-02-06: český breviář nemá vlastný hymnus
		...
		case 25: // MES_JAN -- 25JAN
			if (poradie_svaty == 1){
				...
				// 2008-02-13: doplnené mcd pre českú verziu; 2008-02-22 aj pre slovenskú
				_vlastna_cast_mcd_ant_kcitresp_modl;
		...
		case 28: // MES_JAN --28JAN
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				if (poradie_svaty == 1){
					...
					modlitba = MODL_POSV_CITANIE;
					// 2012-02-06: vlastná psalmódia: Žalm 40, 2–14. 17–18; Žalm 27, 1-6 (chybne v tlačenom propriu uvedené na str. 42: 1–9)
				...
				if (_global_jazyk == JAZYK_CZ_OP){
					...
					// 2010-03-26: pre dominikánov samostatne
					if (poradie_svaty == 2){
						// definovanie parametrov pre modlitbu
						if (query_type != PRM_DETAILY)
							set_spolocna_cast(sc, poradie_svaty);
	...
	case MES_FEB:
		...
		case 2: // MES_FEB -- 02FEB
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				// kompletórium: prevzaté podľa 06AUG
				// keďže v liturgicke_obdobie() bolo odvetvené, nič sa pre kompletórium nenastavovalo - doplnené
				modlitba = MODL_KOMPLETORIUM;
			...
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				...
				// kompletórium: 2013-02-03: doplnené | 2013-10-25: len pre slávnosť!
				modlitba = MODL_PRVE_KOMPLETORIUM;
				_set_kompletorium_slavnost(modlitba);
		...
		case 3: // MES_FEB -- 03FEB
			...
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				...
				_global_svaty4.kalendar = KALENDAR_SK_OP;
				pocet = 5;
				...
				_global_svaty5.kalendar = KALENDAR_SK_OP;
			}// kalendár pre KALENDAR_SK_OP
			break;
		...
		case 5: // MES_FEB -- 05FEB
			// 2011-02-18: sv. agaty je aj vo zväzku II, preto som to obalil "pôstnou spomienkou"
			if (poradie_svaty == 1){
		...
		case 6: // MES_FEB -- 06FEB
			...
			_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_VIAC_MUCENIKOV);
			_global_svaty1.farba = LIT_FARBA_CERVENA;
			// 2010-12-07: pridané pre jezuitov
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SJ)){
		...
		case 8: // MES_FEB -- 08FEB
			...
			// 2006-08-08: spoločná časť pridaná podľa mailu don Valábka z 19. augusta 2005
		...
		case 9: // MES_FEB -- 09FEB
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				...
				sprintf(_anchor_head, "%02d%s_", 10, nazov_MES[mesiac - 1]); // MES_FEB
				Log("  _anchor_head == %s\n", _anchor_head);
		...
		case 14: // MES_FEB -- 14FEB
			...
			if (_global_jazyk == JAZYK_SK){
				...
				// 2010-11-22: český breviár nemá možnosť pre duch. pastierov "pre viacerých" -- MODL_SPOL_CAST_DUCH_PAST_VIACERI // nie je to tu treba, lebo je to vetva len pre SK; ponechané historicky
				if ((_global_jazyk != JAZYK_CZ) && (_global_jazyk != JAZYK_CZ_OP)){
		...
		case 18: // MES_FEB -- 18FEB
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				...
				sprintf(_anchor_head, "%02d%s_", 17, nazov_MES[mesiac - 1]); // MES_FEB
				Log("  _anchor_head == %s\n", _anchor_head);
	...
	case MES_MAR:
		...
		case 4: // MES_MAR -- 04MAR
			...
			// 2011-03-07: opravené na ľubovoľnú spomienku
			_global_svaty1.typslav = SLAV_LUB_SPOMIENKA;
			_global_svaty1.smer = 12; // ľubovoľné spomienky
		...
		case 9: // MES_MAR -- 09MAR
			...
			_global_svaty1.kalendar = KALENDAR_VSEOBECNY;
			break;
			// 2011-02-18: do 9. marca sú propriá vo zväzku III (cezročné obdobie), potom sa pokračuje až dňom 12. mája; zrušené preto časti "else" vetiev if(je_post), lebo nie sú potrebné
		case 10: // MES_MAR -- 10MAR
			...
			// else
			if (_global_jazyk == JAZYK_SK){
				// 2010-03-16: pridané len pre Slovensko; 2011-03-07: opravené pre SK ("termín výročia posviacky katedrálneho chrámu sa bude každoročne sláviť 10. marca. P. Brunovský")
				if (poradie_svaty == 1){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
		...
		case 15: // MES_MAR -- 15MAR
			// 2008-03-12: škaredé nadrátované riešenie, bude potrebné naimplementovať krajšie
			// 2012-02-10: pokus o krajšie riešenie; zmena podmienky a presunutie vyššie...
		...
		label_19_MAR :
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				// 2006-02-04: ak je modlitba cez deň na slávnosť, tak by sa mali použiť žalmy z doplnkovej psalmódie
				if (_global_den.denvt != DEN_NEDELA) {
					_set_zalmy_mcd_doplnkova_psalmodia();
				}
				else {
		...
		label_25_MAR :
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				// 2009-03-23: boli tu žalmy z krstu krista pána: _set_zalmy_sviatok_krstu(modlitba); majú byť podľa narodenia pána
		...
		case 26: // MES_MAR -- 26MAR
			if (_global_den.denvt == DEN_PONDELOK){
				...
				// 2012-02-10: môže sa stať, že pre dátum VN 25. marca je 26.3. pondelok (Veľkonočný pondelok); vtedy sa ale Zvestovanie prekladá podľa podmienky na začiatku marca
				sprintf(_anchor_head, "%02d%s_", den - 1, nazov_MES[mesiac - 1]);
				Log("  _anchor_head == %s\n", _anchor_head);
	...
	case MES_APR:
		...
		case 2: // MES_APR -- 02APR
			if (poradie_svaty == 1){
				...
				if (je_privileg){
					...
					modlitba = MODL_RANNE_CHVALY;
					// 2013-05-13: LA LH, vol. II, p. 1388-1399, uvádza ant. aj pre Veľkonočné obdobie
					_vlastna_cast_benediktus;
					_vlastna_cast_modlitba;
		...
		case 5: // MES_APR -- 05APR
			// 2009-03-25: pre Slovensko, Česko atď. podľa všeobecného kalendára sv. Vincenta Ferrera, pre dominikánov presunuté na 5.5.... 
			// 2009-05-15: aj pre dominikánov platí všeobecný kalendár
			if (poradie_svaty == 1){
				// na spomienku v privilegovaný deň (spomienka v pôste)
				if (je_privileg){
					if (query_type != PRM_DETAILY)
		...
		case 21: // MES_APR -- 21APR
			...
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
				...
				if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
					...
					_global_svaty2.typslav = SLAV_SPOMIENKA;
					_global_svaty2.smer = 10; // mieste povinné spomienky podľa miestneho kalendára; technicky 10, hoci podľa smerníc 11
				}
				pocet = 2;
		...
		case 23: // MES_APR -- 23APR
			...
			if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP) || (_global_jazyk == JAZYK_HU)){
				if (poradie_svaty == 1){
					...
					// 2008-12-10: doplnené pre JAZYK_CZ_OP - vlastný hymnus a niektoré časti pre rch a v
					...
					if ((_global_jazyk == JAZYK_CZ_OP) || ((_global_jazyk == JAZYK_CZ))){
						// 2009-08-27: doplnené modlitby cez deň
						_vlastna_cast_mcd_kcitresp_modl;
					}
				...
				if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
					...
					// 2009-04-23: oprava: už nie je ľubovoľná spomienka, ale povinná spomienka
					_global_svaty1.typslav = SLAV_SPOMIENKA;
					_global_svaty1.smer = 10; // povinné spomienky podľa všeobecného kalendára
				}
		...
		case 29: // MES_APR -- 29APR
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				...
				if (poradie_svaty == 1){
					...
					// _vlastna_cast_mcd_kcitresp_modl; // krátke čítanie a krátke resp. zo spoločnej časti (2013-04-29)
					_vlastna_cast_mcd_modlitba;
		...
		case 30: // MES_APR -- 30APR
			// 2009-03-24: upravené; dominikáni majú památku
			// 2012-04-02: doplnené SK OP
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				if (poradie_svaty == 1){
			...
			if (_global_jazyk == JAZYK_CZ_OP){
				// 2009-03-24: doplnené odvetvenie pre dominikánov;
				// 2010-03-15: pre dominikánov samostatne
				if (poradie_svaty == 3){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
	...
	case MES_MAY:
		...
		case 1: // MES_MAY -- 01MAJ
			if (poradie_svaty == 1){
				...
				if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
					// 2009-07-27: doplnené premenné pom_den a pom_mesiac na uchovanie pôvodného dňa a mesiaca pre český breviář sa totiž berú niektoré časti z 19. marca 
					pom_den = den;
					pom_mesiac = mesiac;
					den = 19;
					mesiac = MES_MAR + 1;
				...
				if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
					// 2009-07-27: doplnené premenné pom_den a pom_mesiac na uchovanie pôvodného dňa a mesiaca pre český breviář sa totiž berú niektoré časti z 19. marca 
					// -- v tejto časti sa hodnoty upravujú späť na pôvodné --
					den = pom_den;
					mesiac = pom_mesiac;
		...
		case 5: // MES_MAY -- 05MAJ
			// 2009-03-25: pre dominikánov presunuté z 5.4. zo všeobecného kalendára sv. Vincenta Ferrera...
			if (_global_jazyk == JAZYK_CZ_OP){
				if (poradie_svaty == 1){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
		...
		case 11: // MES_MAY -- 11MAJ
			...
			break;
			// 2011-02-18: do 9. marca sú propriá vo zväzku III (cezročné obdobie), potom sa pokračuje až dňon 12. mája
		case 12: // MES_MAY -- 12MAJ
			...
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OFMCAP)){
				...
				_global_svaty4.smer = 10; // mieste povinné spomienky podľa miestneho kalendára; technicky 10, hoci podľa smerníc 11
				pocet = 4;
				...
				_global_svaty4.kalendar = KALENDAR_CZ_OFMCAP;
			}// kalendár pre KALENDAR_CZ_OFMCAP
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				if (poradie_svaty == 3){
		...
		case 14: // MES_MAY -- 14MAJ
			if (_global_jazyk != JAZYK_HU){
				if (poradie_svaty == 1){
					...
					modlitba = MODL_RANNE_CHVALY;
					if (_global_jazyk == JAZYK_CZ){
						// 2008-05-14: doplnené - pre českú verziu vlastný, pre slovenskú rovnaký ako na posv. čítanie 
						// 2011-05-14: latinský ani slovenský neurčuje, že sa pre RCH má brať vlastný hymnus; ten je len pre posv. čítanie
						_vlastna_cast_hymnus(modlitba, _global_den.litobd);
					}
					_vlastna_cast_modlitba;
					_vlastna_cast_benediktus;
					...
					modlitba = MODL_VESPERY;
					if (_global_jazyk == JAZYK_CZ){
						_vlastna_cast_hymnus(modlitba, _global_den.litobd); // 2009-07-27: doplnené; je vlastný hymnus, resp. klasický obohatený o 4. sloku
					}
					_vlastna_cast_modlitba;
					_vlastna_cast_magnifikat;
		...
		case 18: // MES_MAY -- 18MAJ
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
				...
				if (poradie_svaty == 1){
					...
					modlitba = MODL_POSV_CITANIE;
					_vlastna_cast_full(modlitba);
					// 2011-03-17: doplnené žalmy, ktoré zatiaľ nikde inde nie sú v tomto radení použité
					...
					modlitba = MODL_VESPERY;
					_vlastna_cast_full(modlitba);
					// 2011-03-17: doplnené žalmy, ktoré sú len pre prvé vešpery sviatku duchovných pastierov, _set_zalmy_sviatok_duch_past()
		...
		case 20: // MES_MAY -- 20MAJ
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
				...
				// 2006-09-12: pôvodne bola ľubovoľná spomienka na Sv. Klementa Marie Hofbauera - od 29.8.1991 památka pre Moravu; "v čechách si lidé mohou vybrat buď Klementa nebo Bernardina na Moravě máme předepsaného jenom Klementa." - p. Franta, 2008-05-21
				else if (poradie_svaty == 2){
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
		...
		case 21: // MES_MAY -- 21MAJ
			if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
				...
				_global_svaty1.smer = 11; // mieste povinné spomienky
			...
			// 2009-07-30: pridané - Sv. Krištofa Magallanesa, kňaza a spoločníkov, mučeníkov
			if (_global_jazyk == JAZYK_CZ_OP){
				if (poradie_svaty == 3){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
				...
				// 2010-11-22: český breviár nemá možnosť pre duch. pastierov "pre viacerých"
		...
		case 24: // MES_MAY -- 24MAJ
			...
			// 2010-05-24: doplnené podľa toho, čo sa má brať z vlastnej časti
			// 2011-03-17: doplnené pre SK OFM
			if ((_global_jazyk == JAZYK_CZ_OP) || ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP))){
				if (poradie_svaty == 1){
		...
		case 27: // MES_MAY -- 27MAJ
			...
			_global_svaty1.kalendar = KALENDAR_VSEOBECNY;
			// 2006-09-14: pridané;
			// 2010-12-16: presunuté z 2. decembra
			if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
				if (poradie_svaty == 2){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
		...
		case 30: // MES_MAY -- 30MAJ
			...
			// 2009-03-24: doplnené pre dominikánov aj český breviář
			// 2012-08-15: doplnené aj pre SK OP
			...
			if (_global_jazyk == JAZYK_CZ){
				// 2012-07-23: odvetvené pre českú verziu
				if (poradie_svaty == 2){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
	...
	case MES_JUN:
		...
		case 6: // MES_JUN -- 06JUN
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				if (poradie_svaty == 1){
					...
					// 2010-10-11: ak je modlitba cez deň na slávnosť, tak by sa mali použiť žalmy z doplnkovej psalmódie
					if (_global_den.denvt != DEN_NEDELA) {
						_set_zalmy_mcd_doplnkova_psalmodia();
					}
					else {
				...
				// od 29.8.1991 památka podľa českého breviára z r. 1995, ale novšie kalendáre uvádzajú nezávaznú památku
				// 2008-06-09: podľa Adama Sýkoru by mala byť aj pre Slovensko len ľubovoľná spomienka; zmenené
				_global_svaty1.typslav = SLAV_LUB_SPOMIENKA;
		...
		case 11: // MES_JUN -- 11JUN
			if (poradie_svaty == 1){
				// definovanie parametrov pre modlitbu
				// 2005-08-27: sviatok apoštola Barnabáša - žalmy a antifóny zo dňa - ale len pre ranné chvály; 
				// preto vlastne to ignorujeme, nech sa napr. posvätné čítanie alebo vešpery berú z apoštolov - preto sme zapoznámkovali nasledovné priradenie:
				// _global_opt 2 = MODL_ZALMY_ZO_DNA; - žalmy a antifóny zo dňa - ale len pre ranné chvály 
		...
		case 12: // MES_JUN -- 12JUN
			...
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SDB)) || ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_SDB))){
				// 2014-06-04: CZ má ako poradie == 1 text_JUN_12_CZ
				...
				_global_svaty(1 + je_cz).typslav = SLAV_SPOMIENKA;
				_global_svaty(1 + je_cz).smer = 11; // miestne povinné spomienky
				mystrcpy(_global_svaty(1 + je_cz).meno, text_JUN_12_SDB[_global_jazyk], MENO_SVIATKU);
				_global_svaty(1 + je_cz).spolcast = _encode_spol_cast(MODL_SPOL_CAST_VIAC_MUCENIKOV);
				_global_svaty(1 + je_cz).farba = LIT_FARBA_CERVENA;
		...
		case 16: // MES_JUN -- 16JUN
			if (_global_jazyk == JAZYK_SK){
				// 2005-07-27: pridané, je to spomienka len v rožňavskej katedrále
				if (poradie_svaty == 1){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
		...
		case 19: // MES_JUN -- 19JUN
			if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_HU)){ // 2006-09-12: odvetvené; 2011-06-07: pridané aj HU
				if (poradie_svaty == 1){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
				...
				// 2008-06-19: zmenené, povinná spomienka len v čb-diecéze
		...
		case 21: // MES_JUN -- 21JUN
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SJ)){
				...
				_global_svaty1.typslav = SLAV_SPOMIENKA;
				_global_svaty1.smer = 10; // miestne povinné spomienky podľa vľeobecného kalendára -- 10 preto, lebo je to aj vo všeobecnom kalendári
		...
		case 25: // MES_JUN -- 25JUN
			// hoci slavnost narodenia sv. Jana Krstitela padla na nedelu, slavi sa v nedelu, lebo ide o nedelu v obdobi cez rok, ktora ma nizsiu prioritu, preto to, co tu bolo 
			//	if(_global_den.denvt == DEN_PONDELOK){
			//		sprintf(_anchor_head, "%02d%s_", den - 1, nazov_MES[mesiac - 1]);
			//		Log("  _anchor_head == %s\n", _anchor_head);
		...
		case 30: // MES_JUN -- 30JUN
			...
			// hoci slavnost sv. Petra a Pavla padla na nedelu, slavi sa v nedelu, lebo ide o nedelu v obdobi cez rok, ktora ma nizsiu prioritu, preto to, co tu bolo 
			...
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_CSSR)){
				...
				_global_svaty3.spolcast = _encode_spol_cast(MODL_SPOL_CAST_DUCH_PAST_KNAZ, MODL_SPOL_CAST_SV_MUZ_REHOLNIK);
				_global_svaty3.farba = LIT_FARBA_BIELA;
				_global_svaty3.kalendar = KALENDAR_CZ_CSSR;
			}// kalendár pre KALENDAR_CZ_CSSR
			break;
		}; break; // case MES_JUN
		// --------------- 07 júl ---------------
	case MES_JUL:
		...
		case 4: // MES_JUL -- 04JUL
			if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_HU) || (_global_jazyk == JAZYK_LA)){
				// 2009-07-09: odvetvené len pre SK; 2009-08-25: je aj pre cz+czop, ale až ako 2. v poradí 
				// 2011-06-30: pridané aj HU a LA
				if (poradie_svaty == 1){
					// definovanie parametrov pre modlitbu
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
		...
		case 5: // MES_JUL -- 05JUL
			if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
				...
				_global_svaty1.typslav = SLAV_SLAVNOST;
				_global_den.typslav = SLAV_SLAVNOST; // 2010-07-06: doplnené kvôli správnemu fungovaniu set_spolocna_cast(), t. j. tamojších kontrol, či napr. sú žalmy vlastné
				_global_svaty1.smer = 3; // slávnosti Pána, preblahoslavenej Panny Márie a svätých, uvedené vo všeobecnom kalendári
		...
		case 6: // MES_JUL -- 06JUL
			...
			_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_MUCENICA, MODL_SPOL_CAST_PANNA);
			// hoci slavnost sv. Cyrila a Metoda padla na nedelu, slavi sa v nedelu, lebo ide o nedelu v obdobi cez rok, ktora ma nizsiu prioritu, preto to, co tu bolo 
		...
		case 11: // MES_JUL -- 11JUL
			...
			if (poradie_svaty == 1){
				...
				_vlastna_cast_benediktus
					if (_global_jazyk == JAZYK_SK){
						_vlastna_cast_prosby;
					}// 2008-07-11: české má zo spoločnej časti: o sv. mužích nebo o řeholnících
				if (_global_jazyk == JAZYK_CZ_OP){
					_vlastna_cast_kresponz;
				}// 2011-05-23: české CZOP má v spoločnej časti: o sv. mužích odlišné ako vo vlastnej časti...
				_vlastna_cast_modlitba;

				modlitba = MODL_POSV_CITANIE;
				_vlastna_cast_hymnus(modlitba, _global_den.litobd); // 2006-02-02: doplnené; hymnus ako na vešpery
				_vlastna_cast_modlitba;
				_vlastna_cast_2citanie;
		...
		case 14: // MES_JUL -- 14JUL
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				...
				if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OFMCAP)){
					...
					_global_svaty3.spolcast = _encode_spol_cast(MODL_SPOL_CAST_DUCH_PAST_KNAZ, MODL_SPOL_CAST_SV_MUZ_REHOLNIK);
					_global_svaty3.farba = LIT_FARBA_BIELA;
					_global_svaty3.kalendar = KALENDAR_CZ_OFMCAP;
				}// kalendár pre KALENDAR_CZ_OFMCAP
			}// všeobecný kalendár a špecifické prípady odlišné od OPRAEM
			break;
		...
		case 16: // MES_JUL -- 16JUL
			if (poradie_svaty == 1){
				...
				modlitba = MODL_POSV_CITANIE;
				_vlastna_cast_modlitba;
				_set_zalmy_sviatok_marie(modlitba); // 2009-09-18, doplnené
				_vlastna_cast_2citanie;
		...
		case 22: // MES_JUL -- 22JUL
			if (poradie_svaty == 1){
				...
				_vlastna_cast_hymnus(modlitba, _global_den.litobd);
				_vlastna_cast_antifony;
				// 2009-07-29: doplnené žalmy z nedele 1. týždňa
				_set_zalmy_1nedele_rch();
				_vlastna_cast_kresponz;
				_vlastna_cast_kcitanie; // 2009-07-29: má byť rovnaké ako zo spoločnej časti svätých žien
				_vlastna_cast_benediktus;
				if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
					_vlastna_cast_prosby;
				}
				...
				_vlastna_cast_hymnus(modlitba, _global_den.litobd);
				_vlastna_cast_antifony;
				// 2009-07-29: doplnené žalmy zo spoločnej časti sv. žien
				_set_zalmy_sviatok_panien(modlitba);
				_vlastna_cast_kresponz;
				_vlastna_cast_kcitanie; // 2009-07-29: má byť rovnaké ako zo spoločnej časti svätých žien
				_vlastna_cast_magnifikat;
		...
		case 23: // MES_JUL -- 23JUL
			...
			_global_svaty1.typslav = SLAV_SVIATOK;
			// _global_svaty1.smer = 12;
			// zmena, na ktoru ma mailom zo dna Monday, July 23, 2001 11:40 AM upozornil Marian Valábek,SDB <marian.valabek@donbosco.sk>:
			// "Prosím opravte Sv. Brigita je spolu so sv. Katarínou Sienskou a sv. Teréziou Benediktou z Kríza spolupatrónkou Európy a teda je to v Európe sviatok od 1999." | 07/09/2001A.D.
		...
		case 26: // MES_JUL -- 26JUL
			...
			// 2006-08-07: upravené lebo žili v manželstve :) pridaná možnosť aj pre sv. ženy - 2006-08-19
			_global_svaty1.farba = LIT_FARBA_BIELA;
			_global_svaty1.kalendar = KALENDAR_VSEOBECNY;
			if (_global_jazyk == JAZYK_HU){
		...
		case 27: // MES_JUL -- 27JUL
			if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
				...
				if (_global_jazyk == JAZYK_SK){
					// 2010-07-27: slovenský breviár má: zo spol. časti svätých mužov
					_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_SV_MUZ_VIACERI);
					_global_svaty1.kalendar = KALENDAR_VSEOBECNY_SK;
				}
				else{
					// 2010-07-27: český breviář má: spol. texty o duchovních pastýřech
					// 2010-11-22: český breviár nemá možnosť pre duch. pastierov "pre viacerých" -- MODL_SPOL_CAST_DUCH_PAST_VIACERI
					_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_DUCH_PAST_KNAZ);
					_global_svaty1.kalendar = KALENDAR_VSEOBECNY_CZ;
				}
		...
		case 30: // MES_JUL -- 30JUL
			// 2004-08-14: Sviatok Bl. Zdenky Schelingovej, panny a mučenice, ako 2. možnosť | Upozornil Jan Kusenda <exonkuda@savba.sk>.
			if (poradie_svaty == 1){
				// definovanie parametrov pre modlitbu
				if (query_type != PRM_DETAILY)
					set_spolocna_cast(sc, poradie_svaty);
	...
	case MES_AUG:
		...
		case 2: // MES_AUG -- 02AUG
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
				...
				if (poradie_svaty == 1){
					...
					_global_svaty3.spolcast = _encode_spol_cast(MODL_SPOL_CAST_DUCH_PAST_KNAZ);
					_global_svaty3.farba = LIT_FARBA_BIELA;
					_global_svaty3.kalendar = (_global_jazyk == JAZYK_SK) ? KALENDAR_SK_SDB : KALENDAR_CZ_SDB;
				}// kalendár pre KALENDAR_SK_SDB a KALENDAR_CZ_SDB
				else if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SJ)){
					if (poradie_svaty == 3){
		...
		case 6: // MES_AUG -- 06AUG
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
					...
					// 2013-08-06: žalmy sú z bežného dňa (LH, zv. IV, str. 1306)
					_vlastna_cast_mcd_ant_kcitresp_modl;
				}
				...
				// 2009-01-06: keďže v liturgicke_obdobie() bolo odvetvené, nič sa pre kompletórium nenastavovalo - doplnené
				modlitba = MODL_KOMPLETORIUM;
				// 2011-08-24: ak padne tento sviatok na sobotu, kompletórium (po druhých vešperách) má byť nedeľné, po prvých vešperách
				if (_global_den.denvt == DEN_SOBOTA){
					zaltar_kompletorium(DEN_NEDELA /* _global_den.denvt */, _global_den.litobd /* OBD_CEZ_ROK */, ZALTAR_VSETKO /* specialne */, _global_den.tyzzal);
					_global_modl_kompletorium = _global_modl_prve_kompletorium;
				}// DEN_SOBOTA
		...
		case 7: // MES_AUG -- 07AUG
			if (poradie_svaty == 1){
				...
				// 2010-08-03: doplnené pre dominikánov
		...
		case 8: // MES_AUG -- 08AUG
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
				...
				if (_global_jazyk == JAZYK_CZ_OP){
					// 2010-03-22: pre dominikánov samostatne
					if (poradie_svaty == 2){
						// definovanie parametrov pre modlitbu
						if (query_type != PRM_DETAILY)
							set_spolocna_cast(sc, poradie_svaty);
		...
		case 9: // MES_AUG -- 09AUG
			...
			// _global_svaty1.smer = 12;
			// zmena, na ktoru ma mailom zo dna Monday, July 23, 2001 11:40 AM upozornil Marian Valábek,SDB <marian.valabek@donbosco.sk>:
			// "Prosím opravte Sv. Brigita je spolu so sv. Katarínou Sienskou a sv. Teréziou Benediktou z Kríza spolupatrónkou Európy a teda je to v Európe sviatok od 1999." | 26/09/2001A.D.
			// 2006-01-11: opravené, do názvu pridané: ", spolupatrónky Európy"
			_global_svaty1.smer = 10; // povinné spomienky podľa všeobecného kalendára
		...
		case 12: // MES_AUG -- 12AUG
			// 2004-08-14: Sviatok Sv. Jany Františky de Chantal presunuty z 12. decembra na 12. augusta.
			if (poradie_svaty == 1){
				// definovanie parametrov pre modlitbu
				if (query_type != PRM_DETAILY)
					set_spolocna_cast(sc, poradie_svaty);
		...
		case 20: // MES_AUG -- 20AUG
			if (_global_jazyk == JAZYK_HU){
				// 2011-07-22: prevzaté podľa 05JUL
				_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_APOSTOL);
				_global_opt[OPT_3_SPOLOCNA_CAST] = _decode_spol_cast(_global_svaty1.spolcast).a1;
				_global_svaty1.typslav = SLAV_SLAVNOST;
				_global_den.typslav = SLAV_SLAVNOST; // 2010-07-06: doplnené kvôli správnemu fungovaniu set_spolocna_cast(), t. j. tamojších kontrol, či napr. sú žalmy vlastné
				...
				if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
					...
					modlitba = MODL_POSV_CITANIE;
					_vlastna_cast_full(modlitba);
					// 2011-07-22: žalmy vlastné, ktoré zatiaľ nikde inde nie sú v tomto radení použité
		...
		case 25: // MES_AUG -- 25AUG
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))
				...
				if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_SDB)){
					...
					_global_svaty5.kalendar = KALENDAR_CZ_SDB;
				}// KALENDAR_CZ_SDB
			}
			break;
	...
	case MES_SEP:
		...
		case 3: // MES_SEP -- 03SEP
			if (poradie_svaty == 1){
				...
				modlitba = MODL_RANNE_CHVALY;
				// v slovenskom breviári má vlastný hymnus (ranné chvály; vešpery), v českom nie; 2008-09-10; aj v dominikánskom má vlastný hymnus; 2009-09-09; aj v maďarskom
				if (_global_jazyk != JAZYK_CZ)
					_vlastna_cast_hymnus(modlitba, _global_den.litobd);
				_vlastna_cast_benediktus;
				_vlastna_cast_modlitba;
		...
		case 7: // MES_SEP -- 07SEP
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SJ)){
				...
				if (_global_jazyk == JAZYK_HU){ // 2011-08-23: pridané aj HU | 2012-09-12: pre HU sviatok
					...
					// 2006-09-12: odvetvené pre Česko 
					// 2008-11-29: pridané pre czop - dominikáni
					_global_svaty1.typslav = SLAV_LUB_SPOMIENKA;
					_global_svaty1.smer = 12; // ľubovoľné spomienky
					_global_svaty1.kalendar = KALENDAR_VSEOBECNY_CZ;
				}
		...
		case 12: // MES_SEP -- 12SEP
			// 2006-09-12: bolo odvetvené len pre Slovensko;
			// 2008-09-23: pre Česko je taktiež ľubovoľnou spomienkou
			if (poradie_svaty == 1){
				// definovanie parametrov pre modlitbu
				if (query_type != PRM_DETAILY)
					set_spolocna_cast(sc, poradie_svaty);
		...
		case 14: // MES_SEP -- 14SEP
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				*/
				...
				// 2006-02-07: doplnené mcd; žalmy pre mcd sú zo dňa, vyriešené pomocou ZALTAR_IBA_ZALMY_HYMNUS_MCD
				_vlastna_cast_mcd_ant_kcitresp_modl;
		...
		case 15: // MES_SEP -- 15SEP
			...
			if (((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) && (_global_jazyk == JAZYK_SK)) || (poradie_svaty == 1)){
				...
				// 2009-07-10: nastavenie pre český breviár - je to len spomienka | 2013-10-25: teda presnejšie: iba pre SK je to slávnosť
				if (_global_jazyk == JAZYK_SK){
					modlitba = MODL_PRVE_VESPERY;
					_vlastna_cast_full(modlitba);
					_set_zalmy_sviatok_panien(modlitba);
				...
				modlitba = MODL_POSV_CITANIE;
				// 2009-07-10: nastavenie pre český breviár - je to len spomienka
				if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_HU)){
					_vlastna_cast_full(modlitba);
				}
				else{
				...
				// 2009-09-17: nastavenie pre český breviár - je to len spomienka
				if (_global_jazyk == JAZYK_SK){
					_vlastna_cast_mcd_ant_kcitresp_modl;
			...
			if (_global_jazyk == JAZYK_SK){
				// 2006-09-12: odvetvené pre Slovensko
				_global_svaty1.typslav = SLAV_SLAVNOST;
				_global_svaty1.smer = 3; // slávnosti Pána, preblahoslavenej Panny Márie a svätých, uvedené vo všeobecnom kalendári
			}
		...
		case 24: // MES_SEP -- 24SEP
			if (_global_jazyk == JAZYK_SK){
				...
				// 2013-07-29, oprava: nie je slávnosť vo všeobecnom maďarskom kalendári, ale len sviatok | upozornila Julianna Blahó
				if (poradie_svaty == 1){
					if (query_type != PRM_DETAILY)
						set_spolocna_cast(sc, poradie_svaty);
		...
		case 26: // MES_SEP -- 26SEP
			// 2009-03-24: pre Slovensko, Česko atď. podľa všeobecného kalendára sv. Kozmu a Damiána, pre dominikánov z 28.9. mučedníkov...
			// 2009-05-15: aj pre dominikánov platí všeobecný kalendár
			// 2010-10-11: pre CSSR_SK: aj ľubovoľná spomienka Bl. Gašpara Stanggassingera, kňaza
			if (poradie_svaty == 1){
				// definovanie parametrov pre modlitbu
				if (query_type != PRM_DETAILY)
					set_spolocna_cast(sc, poradie_svaty);
		...
		case 28: // MES_SEP -- 28SEP
			if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){ // 2008-10-09: odvetvené len pre Česko; prevzaté podľa cyrila a metoda
				_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_MUCENIK);
				_global_opt[OPT_3_SPOLOCNA_CAST] = MODL_SPOL_CAST_MUCENIK; // 2009-10-27: doplnené, aby sa to mohlo použiť v set_spolocna_cast()
				_global_svaty1.typslav = SLAV_SLAVNOST;
				_global_svaty1.smer = 3; // slávnosti Pána, preblahoslavenej Panny Márie a svätých, uvedené vo všeobecnom kalendári
				...
				if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
					...
					_vlastna_cast_2citanie;
					_vlastna_cast_modlitba;
					// _set_zalmy_sviatok_muc(modlitba); // 2009-10-27: netreba, zabezpečí predsunutie priradení navrch a volanie set_spolocna_cast()
	...
	case MES_OCT:
		...
		case 8: // MES_OCT -- 08OKT
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OPRAEM)){
				...
				sprintf(_anchor_head, "%02d%s_", 7, nazov_MES[mesiac - 1]); // MES_OKT
				Log("  _anchor_head == %s\n", _anchor_head);
		...
		case 16: // MES_OCT -- 16OKT
			if (((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_CSSR)) || ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_CSSR))){
				...
				if ((_global_jazyk == JAZYK_CZ) || (_global_jazyk == JAZYK_CZ_OP)){
					// 2006-09-19: odvetvené len pre Česko
					// 2008-11-29: pridané czop - dominikánský breviář
	...
	case MES_NOV:
		...
		case 1: // MES_NOV -- 01NOV
			...
			if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
				...
				// 2012-11-07: pre dominikánov môžu byť niektoré časti vlastné
				if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP)){
					file_name_vlastny_kalendar(_global_kalendar);
		...
		case 2: // MES_NOV -- 02NOV
			...
			_global_svaty1.typslav = SLAV_SPOMIENKA;
			// 2010-01-08: ak padne toto slávenie na nedeľu, berie sa nedeľné ofícium
			if (_global_den.denvt != DEN_NEDELA){
		...
		label_03NOV :
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OFMCAP)){
				// 2014-01-22/2013-08-19: alebo keď je v tento deň prekážka, tak najbližší voľný deň (vyriešené cez label_03NOV)
				if (poradie_svaty == 1){

					sprintf(_anchor_head, "%02d%s_", 2, nazov_MES[mesiac - 1]); // 2. čítanie: 02NOV (použité natvrdo kvôli prípadnému prekladaniu na 05NOV)
					Log("  _anchor_head == %s\n", _anchor_head);
					modlitba = MODL_POSV_CITANIE;
					_vlastna_cast_2citanie;
					sprintf(_anchor_head, "%02d%s_", 3, nazov_MES[mesiac - 1]); // späť: 03NOV (použité natvrdo kvôli prípadnému prekladaniu na 05NOV)
					Log("  _anchor_head == %s\n", _anchor_head);
		...
		case 5: // MES_NOV -- 05NOV
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_OFMCAP) && (_global_den.denvt == DEN_UTOROK)){
				...
				sprintf(_anchor_head, "%02d%s_", den - 2, nazov_MES[mesiac - 1]);
				Log("  _anchor_head == %s\n", _anchor_head);
			...
			if ((_global_jazyk == JAZYK_CZ) && (_global_kalendar == KALENDAR_CZ_SDB)){
				...
				_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_ZA_ZOSNULYCH);
				_global_svaty1.farba = LIT_FARBA_FIALOVA_CIERNA;
				_global_svaty1.kalendar = KALENDAR_CZ_SDB;
		...
		case 7: // MES_NOV -- 07NOV
			if ((_global_jazyk == JAZYK_CZ_OP) || ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OP))){
				if (poradie_svaty == 1){
					...
					_vlastna_cast_full(modlitba);
					if (_global_jazyk == JAZYK_CZ_OP){
						// 2010-07-09: doplnené žalmy, ktoré zatiaľ nikde inde nie sú v tomto radení použité
						_set_zalmy_pc_1_8_16(modlitba);
					}
					else{// KALENDAR_SK_OP
						// 2012-11-07: doplnené žalmy, ktoré zatiaľ nikde inde nie sú v tomto radení použité
		...
		case 9: // MES_NOV -- 09NOV
			if (poradie_svaty == 1){
				...
				modlitba = MODL_POSV_CITANIE;
				// _vlastna_cast_modlitba; zo spoločnej časti
				_vlastna_cast_2citanie;

				// 2009-01-07: podľa breviára (III., str. 1631) má byť okrem antifón a žalmov (tie sú zo dňa)
				// ostatné zo spoločnej časti [doplnené do spoločnej časti: _set_spolocna_cast()];
				// doplnené, aby modlitba aj pre modlitbu cez deň bola zo spoločnej časti
				// mimochodom, je predpísané Te Deum
				// 2009-11-17: problém: nemajú sa vziať antifóny a žalmy zo spoločnej časti, ale zo dňa... vyriešené v 
				break;
			}
			_global_svaty1.typslav = SLAV_SVIATOK;
			_global_svaty1.smer = 5; // sviatky Pána uvedené vo všeobecnom kalendári
		...
		case 11: // MES_NOV -- 11NOV
			if (poradie_svaty == 1){
				...
				modlitba = MODL_RANNE_CHVALY;
				_set_zalmy_1nedele_rch(); // 2009-07-30: doplnené podľa breviára (ako napísal Vlado Kiš ešte ...)
				...
				modlitba = MODL_VESPERY;
				_set_zalmy_sviatok_duch_past(modlitba); // 2009-07-30: doplnené podľa breviára (ako napísal Vlado Kiš ešte ...)
				if ((_global_jazyk == JAZYK_SK) || (_global_jazyk == JAZYK_HU)){ // 2008-11-29: odvetvené len pre Slovensko | 2011-10-24: pridané aj HU
					_vlastna_cast_full_okrem_prosieb(modlitba);
				}
				else{
			...
			if (_global_jazyk == JAZYK_SK){
				_global_svaty1.typslav_lokal = LOKAL_SLAV_SPIS_BA_PATRON; // 2005-10-28: pridané; 2008-06-24: zmenené z pôvodného LOKAL_SLAV_SPIS_PATRON
			}
			else if (_global_jazyk == JAZYK_HU){
				_global_svaty1.typslav_lokal = LOKAL_SLAV_SZOMBATHELYI_PATRON; // 2011-11-08: pridané
			}
			_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_DUCH_PAST_BISKUP);
			_global_svaty1.farba = LIT_FARBA_BIELA;
			_global_svaty1.kalendar = KALENDAR_VSEOBECNY;
		...
		case 13: // MES_NOV -- 13NOV
			if ((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_SJ)){
				...
				if (poradie_svaty == 1){
					...
					modlitba = MODL_RANNE_CHVALY;
					_set_zalmy_1nedele_rch();
					// 2009-12-08: pre dominikánov prosby zo spol. časti, pre český breviár: z vlastnej časti
					if (_global_jazyk == JAZYK_CZ){
						_vlastna_cast_full(modlitba);
					}
					else{
					...
					// modlitba cez deň, 2009-08-04: pridané | 2009-11-17: odstavené; totiž kedysi bol "sviatok", teraz "spomienka"; upozornil Mgr. Martin Kubeš, notář
					// _vlastna_cast_mcd_kcitresp_modl;
					break;
				}
				_global_svaty1.typslav = SLAV_SPOMIENKA;
		...
		case 17: // MES_NOV -- 17NOV
			if (_global_jazyk == JAZYK_HU){
				...
				_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_SV_ZENA_SKUTKYMIL); // 2010-11-18: v breviári je len: "pre tých, čo konali skutky milosrdenstva"
				_global_svaty1.farba = LIT_FARBA_BIELA;
				_global_svaty1.kalendar = KALENDAR_VSEOBECNY;
			}// všeobecný kalendár
			break;
		...
		case 19: // MES_NOV -- 19NOV
			if (_global_jazyk == JAZYK_HU){
				if (poradie_svaty == 1){
					...
					modlitba = MODL_POSV_CITANIE;
					_vlastna_cast_full(modlitba);
					// 2011-11-08: doplnené žalmy, ktoré zatiaľ nikde inde nie sú v tomto radení použité
		...
		case 21: // MES_NOV -- 21NOV
			if (poradie_svaty == 1){
				...
				modlitba = MODL_RANNE_CHVALY;
				if (_global_jazyk != JAZYK_CZ){ // 2008-11-29: odvetvené pre Slovensko a czop; 2011-11-08: pridané aj HU
					_vlastna_cast_hymnus(modlitba, _global_den.litobd);
				}
				_vlastna_cast_benediktus;
				_vlastna_cast_modlitba;
		...
		case 30: // MES_NOV -- 30NOV
			if (poradie_svaty == 1){
				...
				modlitba = MODL_POSV_CITANIE;
				if (_global_jazyk == JAZYK_CZ){ // 2012-01-09: doplnený vlastný hymnus pre posv. čítanie pre CZ
					_vlastna_cast_hymnus(modlitba, _global_den.litobd);
				}
				_vlastna_cast_modlitba;
				_vlastna_cast_1citanie;
	...
	case MES_DEC:
		...
		label_8_DEC :
			_global_svaty1.spolcast = _encode_spol_cast(MODL_SPOL_CAST_PANNA_MARIA);
					if ((poradie_svaty == UNKNOWN_PORADIE_SVATEHO) || (poradie_svaty == 1)){
						...
						// ktora ma takmer najvacsiu prioritu, a preto ma aj prve vespery - a vtedy by to normalne nefungovalo;
						// nastavenie veci pre modlitbu by sa muselo diat v predoslom dni, co je neciste riesenie
						...
						modlitba = MODL_POSV_CITANIE;
						_vlastna_cast_full(modlitba);
						// 2005-10-24: možno bude potrebné nastaviť žalmy?
						_set_zalmy_sviatok_marie(modlitba);
						// 2006-02-03: celkom dobrá intuícia :) žalmy nastavené

						// 2014-02-13: CZ má vlastný hymnus
						if (_global_jazyk == JAZYK_CZ){
							_vlastna_cast_mcd_full;
						}
						else{
		...
		case 16: // MES_DEC -- 16DEC
			...
			break;
			// odtiaľto až do konca roka sú tzv. privilegované dni
	...
	// 2005-08-25: Pridané vlastné druhé čítanie na posvätné čítanie
#define _srdca_pm_2cit {\
	...
	if ((_global_den.denvr == SRDPM) &&
		...
		// neposkvrneneho srdca panny marie; 10/03/2000A.D.; vtedy som aj napisal tuto vysvetlujucu poznamku:
		// "berie sa len v takom pripade, ked to nekoliduje s inou povinnou spomienkou alebo slavenim, co ma vyssiu prioritu (smer <= 10)"
		// kedze vsak som si uvedomil, ze bolo treba nastavit smer=10, tu treba dat opravenu poznamku:
		// "berie sa v takom pripade, ked nie je slavenie s vyssou prioritou, teda smer < 10"
		// 2003-06-30a.D.
		...
		_global_svaty1.typslav = SLAV_SPOMIENKA;
		// 2003-06-30: kedysi tu bolo 11: "miestne povinne spomienky podla vseobecneho kalendara" zmenil som to na 10
		_global_svaty1.smer = 10;
		// vysvetlujuca poznamka z r. 2000: "zrejme pre Slovensko je tato lubovolna  spomienka povinna; aby sa nebila s inou spomienkou, dal som tam 
		// smer == 11; 10/03/2000A.D.;" -- zmenene 2003-06-30 | porov. pasaz venovanu srdcu pm vo funkcii breviar.cpp::_rozbor_dna()
		if (!((_global_jazyk == JAZYK_SK) && (_global_kalendar == KALENDAR_SK_OFM))){
